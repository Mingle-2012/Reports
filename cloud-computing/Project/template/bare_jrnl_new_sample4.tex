...
\author{Pengda Li, Li Wang, Zekai Wu, Yunbiao Zhang
        % <-this % stops a space
...
\begin{abstract}
As a foundational framework for processing large-scale data across distributed systems, MapReduce has demonstrated significant success in traditional distributed computing environments. However, the rapid evolution of cloud computing introduces new challenges and opportunities for its optimization. This paper revisits the MapReduce framework in the context of distributed cloud computing, analyzing its performance, scalability, and adaptability to heterogeneous and elastic environments. Key limitations, such as data locality, fault tolerance, and resource utilization, are critically examined. Furthermore, we propose enhancements to the framework to better leverage cloud-native capabilities, including dynamic resource allocation, scalability, and cost efficiency. Through theoretical analysis and empirical evaluation, our findings highlight the potential for MapReduce to remain a robust solution in modern cloud ecosystems, while suggesting directions for future research and development.
\end{abstract}
